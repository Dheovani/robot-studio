...
The initial vertical slice now provides a retained procedural scene, semantic component selection, highlighting, a component inspector, hierarchical transform updates, a curated demonstration, and an internal drive-system teaching view using order-independent transparency. A versioned manifest and desktop package loader validate semantic coverage, safe local GLB paths, missing files, and cached package discovery. The isolated Assimp adapter imports GLB scene hierarchies and binds referenced nodes and mesh descendants to `RobotPartId`, rejecting missing or ambiguous names. This proves the application boundary, interaction flow, initial cutaway direction, package contract, and import boundary. It does not yet load an authored Cartesian GLB into the viewport or satisfy PBR asset, representative material-validation, or complete lifecycle acceptance criteria, so the proof of concept remains in progress.
