...
The XY Plotter card also provides `Explore Mechanics`. Its showcase presents a practical pen plotter with a paper bed, dual Y drive, moving bridge, X carriage, pen lift, pen, and controller. The available layers inspect the assembled machine, expose its drive components, overlay the supported X/Y directions, or separate the major assemblies. Use `Rectangular path tour` to inspect coordinated planar motion, `Individual axis inspection` to distinguish X from Y, and the exploded-layer assembly sequence to see how the mechanisms connect.
...
The Differential Drive card also provides `Explore Mechanics`. Its round service-robot model exposes two independent gearmotors, drive wheels and encoders, a passive caster, battery, motion controller, front range sensor, perimeter bumper, and removable shell. The familiar enclosure is visual context only and does not add cleaning behavior. Use `Drive and turn tour` for a closed square route, `Turning comparison` to compare left and right in-place rotation, `Body frame` to follow the robot-local X/Y coordinates, and `Drive system` to inspect and select components through the transparent chassis. The exploded assembly joins the support, power, control, drive, and shell assemblies in mechanical order.
...
The SCARA card also provides `Explore Mechanics`. Its unbranded assembly-arm model uses a compact mounting pedestal, continuous light-colored link covers, an enlarged wrist housing, a blue vertical spindle, and a parallel gripper. Internal components include the pedestal controller, shoulder servo and reduction, structural links, elbow servo, and vertical-axis motor. Use `Pick-and-place cycle` to follow coordinated planar and vertical motion, `Individual joint inspection` to isolate the shoulder, elbow, and spindle, and `Drive system` to select internal components through transparent covers. The joint/tool-axis layer identifies the horizontal work plane and moving Z axis, while the exploded assembly joins the mechanism in mechanical order.
...
The Simple Articulated Arm card also provides `Explore Mechanics`. Its compact desktop serial-arm model exposes a rotating base, shoulder and elbow servo reductions, structural links, protected service routing, wrist coupling, and parallel gripper. Use `Reach and transfer` to inspect coordinated three-axis movement, `Individual joint inspection` to distinguish base yaw from shoulder and elbow pitch, and `Drive system` to select internal components through transparent covers. The joint-axis layer identifies the vertical base axis and two horizontal pitch axes, while the exploded assembly joins the controller, drives, links, wrist, and tool in mechanical order.
...
The Delta Robot card also provides `Explore Mechanics`. Its overhead linear-Delta model presents three servo-driven vertical rails arranged at 120 degrees, three moving carriages, six paired links, a translation-only platform, and a vacuum pick tool. Use `Coupled pick-and-place` to observe synchronized vertical and lateral movement, `Individual actuator inspection` to see why moving one carriage changes the complete platform pose, and `Drive system` to inspect the motors, guides, carriages, and links through the fixed structure. The axis layer compares actuator Z directions with the moving TCP coordinate frame, while the exploded assembly joins the three towers, six links, platform, and tool in mechanical order.
...
The Drone card also provides `Explore Mechanics`. Its compact X-configuration quadcopter model presents a low rounded-rectangle body, four integrated diagonal arms, four visible two-blade propellers, and independently selectable brushless motors. The hull encloses the central airframe, battery, flight controller, and IMU, while the forward camera and landing gear remain visible beneath it. Use `Flight and attitude tour` to observe translation, banking, heading changes, and propeller rotation together. `Counter-rotating motor pairs` isolates the two diagonal pairs that turn in opposite directions to balance reaction torque. The avionics-and-power layer makes the hull transparent and exposes the internal electronics, the axis layer follows the moving body frame, and the exploded assembly joins the aircraft systems in mechanical order.
...
The 6-DOF Industrial Arm card also provides `Explore Mechanics`. Its industrial serial-manipulator model presents a floor pedestal, rotating base, enclosed shoulder and elbow drives, a reinforced upper arm and forearm, three compact wrist axes, external service routing, and a parallel gripper. Use `Coordinated pick tour` to see positioning and orientation axes work together, and `Wrist orientation tour` to isolate J4 forearm roll, J5 wrist bend, and J6 tool roll. The joint-drive layer exposes motors and reductions through transparent covers, the axis layer identifies all six rotations, and the exploded assembly joins the mechanism in serial order.
...
Dropdowns and scrollbars use the RobotStudio dark theme consistently in the catalog, simulation workspaces, glossary, script editors, and mechanical showcases. Dropdowns support keyboard focus and selected-item highlighting, while compact scrollbar thumbs provide distinct hover and drag feedback.
