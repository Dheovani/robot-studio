...
- exposes Cartesian Robot, XY Plotter, Differential Drive Robot, SCARA Robot, Simple Articulated Arm, Delta Robot, Drone, and 6-DOF Industrial Arm as catalog models with mechanical showcases and resolves all eight through the shared showcase catalog.
- validates the SCARA mechanical hierarchy from pedestal through shoulder, elbow, second link, vertical spindle, and tool; verifies at an interpolated mid-motion frame that the first-link endpoint and elbow pivot remain connected; unit-tests revolute-pivot composition; exercises planar and vertical pick-and-place poses; imports every selectable semantic GLB part; and returns the completed exploded assembly to its authored pose.
- validates the Simple Articulated Arm mechanical hierarchy from mounting base through rotating turntable, shoulder, upper arm, elbow, forearm, wrist, and tool; verifies at an interpolated mid-motion frame that shoulder and elbow pivots remain connected; isolates each joint demonstration; imports every selectable semantic GLB part; and returns the completed exploded assembly to its authored pose.
- validates the Delta mechanical hierarchy across three actuator towers, six independently constrained links, one moving platform, and its tool; verifies both endpoints of every link at an interpolated coupled-motion frame; isolates A, B, and C carriage movement; imports every selectable semantic GLB part; and returns the completed exploded assembly to its authored pose.
- unit-tests reusable parallel-link composition by moving carriage and platform endpoints independently and verifying exact attachment at both ends after local/world transform resolution.
- validates the Drone mechanical hierarchy across airframe, arms, motors, and four selectable propellers; requires visible fallback geometry for every propeller; verifies propeller pivots remain aligned with motor axes during interpolated airframe attitude changes; isolates counter-rotating diagonal motor pairs; imports every selectable semantic GLB part; and returns the completed exploded assembly to its authored pose.
- validates the 6-DOF Industrial Arm hierarchy from floor base through six revolute joints and the parallel gripper; verifies all six authored pivots remain connected during interpolated coordinated motion; isolates wrist-orientation behavior; exposes six axis guides; imports every selectable semantic GLB part; and returns the completed exploded assembly to its authored pose.
...
- verifies that the mechanical showcase exposes assembled, drive-system, motion-axis, and exploded-assembly layers, ghosts only components that may obscure the drive system, excludes ghosted structures from drive-system hit testing while preserving selection for internal parts, defines three non-zero X/Y/Z guides with a semantic gantry attachment for the moving X guide, and assigns unique non-zero exploded offsets.
...
- runs a cross-family mechanical-showcase release smoke gate that loads and imports all eight registered GLB packages, resolves the initial semantic selection, samples every teaching view, and resolves complete world-transform hierarchies.
