...
Desktop command hierarchy uses three shared button roles. Primary buttons are reserved for the principal action that advances the current workflow, such as opening or simulating a robot. Secondary buttons support playback and alternative actions without competing with that principal action. Ghost buttons are reserved for navigation and compact tools whose persistent visual weight would add clutter. All three roles share dimensions, typography, keyboard focus, disabled state, and pointer feedback through one base template; screens select a semantic role instead of assigning local colors.
...
Simulation sampling and desktop rendering use independent clocks. Simulation and playback samplers continue producing immutable, deterministic samples at their configured simulation interval. The desktop presentation clock ticks independently at approximately 16 ms, converts real elapsed time and the selected playback speed into simulated playback time, and locates the two surrounding immutable samples. Schematic viewers currently select the nearest sample and recompose a scene only when that selected sample changes; they never advance simulation by counting WPF timer ticks. The selection contract also exposes interpolation progress so a future retained renderer can interpolate transforms without changing snapshot data. Mechanical showcases already use that continuous policy: they sample presentation keyframes by elapsed time and update retained component transforms. Neither path feeds rendered or interpolated state back into domain, motion, or simulation layers.

Visualization performance is measured by a dedicated Release diagnostics executable rather than asserted by timing-sensitive unit tests. The tool profiles every registered GLB through manifest loading, Assimp import, semantic binding, scene-bound propagation, live WPF composition, demonstration transform updates, and Helix hit testing. Reports identify the hardware role and separate one-time native/runtime warm-up from comparable per-model timings. A tested acceptance policy targets 30 FPS at p95 with five-percent WPF scheduler tolerance, no more than 500 ms for warmed GLB import, 50 ms for scene preparation, and 8 ms for transform updates and semantic hit tests; a violation returns a nonzero exit code. Both the [development-workstation baseline](performance-baseline.md) and the [teaching-hardware baseline](performance-baseline-lower-spec.md) pass this policy. The accepted ASUS VivoBook X515JA run uses integrated Intel UHD Graphics and 4 GB RAM, so the current packaged assets and rendering budget are appropriate for that lower-spec teaching profile.

Mechanical showcase views own their Helix effects manager and imported scene. Closing a showcase explicitly stops its presentation timer, detaches WPF and viewport events, removes the imported scene from its host, disposes the scene graph, and releases the effects manager. The same idempotent path also handles WPF `Unloaded`, while the diagnostics gate verifies teardown for every model. GLB scenes are deliberately not retained in a global cache: warmed imports are inexpensive in the measured assets, whereas caching renderer-bound scene nodes would retain GPU resources and complicate ownership.

Educational overlays are renderer-independent presentation data. `RobotStudio.Visualization` defines semantic line, polyline, point, box, and label primitives for coordinate grids and axes, coordinate origins, workspace boundaries, trajectories, positions, physical limits, and future collision bounds. Robot-family composers produce these primitives before Desktop adapters translate them into WPF, Canvas, or Helix geometry. All eight available schematic simulators now use this path for their applicable workspace and trajectory layers, while every mechanical showcase uses it for motion-axis guides. Planar circular, planar rectangular, and volumetric workspace composers preserve the topology appropriate to each robot family. Rendered meshes and overlay controls remain consumers, never sources, of robot or simulation state.

Milestone 5 adds a realistic mechanical-showcase path without replacing the current schematic renderer or deterministic simulation workspace. The schematic workspace remains the executable environment for scripts, commands, planning, and playback. The showcase prioritizes part identification, mechanical relationships, and short predefined demonstrations. A presentation-only demonstration controller may drive renderer-neutral component poses, but rendered meshes must never become the source of domain or simulation state. Graphics-library objects, asset formats, cameras, materials, lighting, hit testing, and rendering frame timing remain desktop concerns.
...
Educational overlays are separate desktop scene objects composed over the retained realistic model. The Cartesian `Motion axes` layer renders color-coded X/Y/Z guides without modifying the GLB or semantic hierarchy. A guide may reference a semantic parent part when it must follow an animated assembly; guides without a parent remain fixed machine-coordinate references. This keeps overlay transforms driven by the same renderer-neutral component poses as the model while preserving the asset as visual data only.

Exploded assembly layers are also presentation-time pose compositions. Robot-specific offset catalogs identify semantic parts and translations in millimeters; the desktop adds those offsets to the active curated demonstration poses and then uses the existing hierarchy resolver. Layer-specific demonstration catalogs prevent unrelated motion tours from appearing in an assembly inspection. A staged assembly sequence uses inverse offsets to return each separated assembly to its authored pose in mechanical order, making reset and playback deterministic and reversible. This preserves inherited movement when appropriate. Exploded offsets never mutate visual-model definitions, packaged assets, domain state, or deterministic simulation snapshots.

Future rendering dependencies must be open source and usable without paid licenses, subscriptions, royalties, or commercial licensing fees. HelixToolkit is the preferred candidate, Stride is the advanced alternative, and Veldrid is a low-level fallback rather than the preferred path. Ab4d.SharpEngine is rejected as the default because its licensing model can introduce commercial licensing requirements. The evaluation must be repeated at the start of Milestone 5 before a dependency is added. glTF 2.0/GLB is the preferred future model asset direction. The complete milestone specification is in [Advanced 3D Visualization](advanced-3d-visualization.md).

The Milestone 5 revalidation selected stable `HelixToolkit.Wpf.SharpDX` 3.1.2 for an isolated Cartesian proof of concept. The matching `HelixToolkit.SharpDX.Assimp` 3.1.2 integration is isolated in the desktop asset importer and does not enter portable projects. Approval remains provisional until combined WPF runtime, PBR material, authored-model loading, hit-testing, transparency or cutaway, retained-transform, and disposal tests pass. Blender is the preferred offline authoring tool, and exported GLB assets must pass the Khronos glTF Validator plus RobotStudio semantic validation. The full evaluation is in [Renderer And Asset Pipeline Evaluation](renderer-evaluation.md).

Each implemented non-Cartesian 3D simulator delegates schematic scene creation to a robot-specific composer. A composer receives an immutable playback snapshot, frame index, and camera orbit values, then returns a complete `SchematicViewportScene` containing camera, workspace, accumulated trajectory, current robot geometry, and optional lighting. `MainWindow` retains playback navigation and text-panel updates but no longer owns those robot geometry rules. This boundary allows scene composition to be tested without constructing the WPF window and prepares overlays to evolve independently from renderer presentation.

The Cartesian Robot and XY Plotter use explicit scene-composer entry points over a shared Cartesian composition core because both consume the same Cartesian playback contract while representing different catalog models. Overlay visibility is captured in an immutable options value before composition; the window supplies the active camera and no longer constructs grids, axes, paths, workspace bounds, labels, carriages, rails, or tools. The Differential Drive simulator follows the same boundary with an immutable `CanvasScene2D`: its composer owns workspace scaling, coordinate mapping, grid, accumulated path, heading, body, and wheel primitives, while a small WPF presenter is solely responsible for materializing those primitives on a `Canvas`. Simulation snapshots remain the source of robot state in both paths.
...
The desktop resolves `MechanicalShowcaseDescriptor.ModelId` through a dedicated showcase catalog. Each registered presentation must match its visual-model id and validates its initial part, view-to-demonstration assignments, semantic overlays, exploded offsets, and fallback primitives before a viewport is created. `MainWindow` hosts the resulting view dynamically and releases it when navigation returns to the robot catalog. The shared view therefore consumes model-specific presentation data instead of constructing the Cartesian definition or locating Cartesian assets itself.

The XY Plotter is the first expansion through this catalog and deliberately reuses the complete mechanical-showcase viewport. Its presentation supplies a robot-specific GLB package, semantic hierarchy, fallback scene, X/Y guides, teaching layers, exploded offsets, and demonstrations. The moving X carriage is parented to the Y gantry, so hierarchy composition produces coordinated planar motion without XY-specific transform logic in the renderer. This confirms that showcase variation belongs in validated presentation data rather than duplicated WPF controls.

The Differential Drive expansion uses the same presentation contract for a mobile topology. The chassis is the visual root and receives planar translation and heading poses; drive units, sensors, electronics, and the caster remain semantic children. Body-frame guides attach to that root and therefore follow both translation and rotation. `RobotPartKind.Wheel` records wheel semantics without adding mobile-robot rules to the renderer, while renderer-only power and sensor material roles improve procedural fallback readability. The showcase animation remains curated presentation data and does not replace differential-drive simulation or odometry state.
...
The Cartesian, XY Plotter, Differential Drive, SCARA, Simple Articulated Arm, Delta Robot, and Drone packages are original technical GLB assets stored with the application and generated reproducibly by `tools/RobotStudio.VisualAssetBuilder`. The utility runs only during asset development; it is not referenced by the application and does not become a rendering or simulation dependency. Its small shared box/cylinder mesh library is sufficient for validating the complete package pipeline and semantic hierarchies. The builder records the first parentless semantic part as the single GLB scene root instead of assuming that every topology names its root `base`. Blender remains the preferred authoring tool when later robot assets require detailed mechanical meshes, UVs, or textures.

The SCARA mechanical showcase is the first realistic-view expansion with multiple revolute transforms in one semantic chain. Its authored geometry remains in one base coordinate system. A shoulder pose rotates the first-link subtree around the pedestal axis. The presentation separately declares the elbow's authored 325 mm pivot; after each sampled rotation, `MechanicalRevoluteJointPoseComposer` calculates the exact translation that preserves that pivot before the elbow inherits shoulder motion. Calculating compensation after interpolation prevents links from separating between keyframes. The vertical spindle then translates along local Z and carries the tool. These curated poses explain the mechanism but remain presentation data; SCARA simulation state, kinematics, and collision rules stay in the portable layers.

The Simple Articulated Arm mechanical showcase extends the same retained hierarchy to a spatial serial arrangement. The turntable rotates around vertical Z, while shoulder and elbow subtrees pitch around authored horizontal Y pivots. Pivot compensation is composed after keyframe interpolation, so both connections remain coincident throughout playback and all downstream parts inherit upstream motion. This curated mechanical presentation intentionally does not redefine the existing planar three-joint simulation or its kinematic contracts; it explains a recognizable physical arrangement while domain and simulation state remain authoritative and renderer-independent.

The Delta mechanical showcase introduces links with two independently moving endpoints, which cannot be represented truthfully by a single-parent scene hierarchy alone. Each parallel-link constraint identifies its semantic link, carriage endpoint, platform endpoint, and authored coordinates. After demonstration sampling, `MechanicalParallelLinkPoseComposer` derives a local similarity transform that maps the authored segment onto the current endpoints, including orientation, length, and translation. This keeps all six links connected during every interpolated frame without giving the renderer ownership of actuator or platform state. Constraints are intentionally not composed in the exploded layer, where separated assemblies must remain independently inspectable. The visual embodiment is an introductory linear Delta consistent with the current deterministic actuator model; it does not claim rotary-Delta dynamics, production tolerances, or flexible-link physics.

The Drone mechanical showcase uses the complete airframe as its semantic root. Translation and roll/pitch/yaw are presentation poses on that root, while arms, motors, propellers, avionics, camera, and landing gear inherit the aircraft transform through the retained hierarchy. Each propeller also has an authored motor-axis pivot and an independent counter-rotating pose, so rotor animation remains centered while the vehicle changes attitude. A low rounded-rectangle hull, integrated diagonal arms, two-blade meshes, and colored tips provide a recognizable consumer-quadcopter silhouette while preserving propeller readability from the initial camera. These curated motions explain quadcopter structure and torque-pair concepts but remain separate from the deterministic Drone simulation and do not model thrust, airflow, motor dynamics, or flight-control loops.

Mechanical teaching views must change the appearance of one retained imported hierarchy instead of loading duplicate models for assembled, cutaway, or highlighted presentations. The drive-system view derives ghosting and technical colors from `RobotPartKind`. Ghosted geometry remains visible but is excluded from hit testing so selection reaches the internal semantic component behind it; the same rule applies to imported assets and procedural fallbacks. The procedural scene is retained only as a deterministic fallback when the packaged asset cannot be loaded.
...
The Cartesian-family desktop sidebar uses presentation-only navigation state to expose one primary area at a time: Script, Control, Monitor, or View. Existing controls and event handlers remain the owners of their current behavior; navigation only changes visibility and restores per-area scroll offsets. Every simulator validates script edits through one short desktop debounce targeted at the editor's robot family, but validation does not capture a simulation session or replace playback. `Simulate` remains the explicit state-changing action. This keeps domain, scripting, planning, and simulation behavior independent of the WPF layout.

Reusable desktop control templates live in a dedicated resource dictionary merged by the main visual resource dictionary. Implicit `ComboBox`, `ComboBoxItem`, and `ScrollBar` styles provide one dark-theme implementation to the catalog, glossary, simulation workspaces, and dynamically hosted mechanical showcases. The templates change presentation only; selection, scrolling, bindings, commands, and viewer state continue to use native WPF contracts.

Desktop visual constants live in `Styles/DesignTokens.xaml`, separately from control templates and view composition. The dictionary defines semantic color roles, control and panel geometry, spacing, a small typography scale, chart-series colors, and renderer-compatible `Color` values; `MainWindowStyles.xaml` consumes those tokens through semantic styles such as section titles, field labels, body copy, and metadata. Tokens are loaded before reusable control templates so dynamically hosted views receive the same values. All desktop XAML consumes these roles instead of declaring local hexadecimal colors, and an automated test preserves that boundary.

The application loads the shared style dictionary at `App` scope because reusable user controls can be parsed before they acquire `MainWindow` as a resource ancestor. Brushes used by visual states inside control-template triggers use dynamic resource lookup, since those template children are created during layout rather than when the dictionary is parsed. This prevents unresolved resources from becoming invalid `UnsetValue` assignments during startup.

Programmatically composed catalog cards resolve the same semantic resources through the window resource dictionary rather than allocating duplicate hard-coded brushes and metrics. This keeps responsive card construction in C# while typography, badges, capability tags, focus colors, and card surfaces remain owned by the theme. The glossary uses the same field, metadata, badge, input, icon-button, and repeated-card styles; search and category filtering behavior remain unchanged.

All simulation workspaces share semantic styles for state labels, state values, state-grid spacing, script field labels, section headings, dividers, and explanatory text. Robot-specific state fields and commands remain in their own viewers, while the common presentation contract prevents family viewers from drifting into separate color and spacing conventions. Cartesian charts and axis indicators use named data-series roles, keeping technical colors distinct from focus, selection, validation, and navigation roles even when their current color values happen to match.

`ViewerHeader` owns the common simulator and mechanical-showcase header composition: title and subtitle typography on the left, with a caller-provided action region on the right. The component does not own navigation or playback behavior; each host supplies its existing buttons and event handlers. This removes duplicated WPF layout without moving workflow state into a generic visual control.

Simulator chrome uses visual hierarchy instead of nested frames: headers and timelines are horizontal bands, sidebars are separated from viewports by one vertical divider, and the viewport remains the primary framed work surface. Resize grips and playback controls retain explicit interaction feedback, while simulation and rendering behavior remain outside the shared visual styles.
...
The separate 6-DOF mechanical showcase uses six semantic revolute components with authored pivots and retained parent-child transforms. J1 turns the complete mechanism, J2 and J3 position the arm, and J4 through J6 form an orientation wrist before the tool flange. The packaged asset approximates a common industrial manipulator through a broad pedestal, enclosed shoulder, reinforced curved upper-arm profile, horizontal forearm, compact wrist stages, external service cable, and parallel gripper. Its curated motion is explanatory presentation data and does not replace the deterministic industrial-arm simulator or define its kinematic state.
