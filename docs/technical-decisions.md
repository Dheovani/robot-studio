...
Educational overlays are renderer-independent presentation data. `RobotStudio.Visualization` defines semantic line, polyline, point, box, and label primitives for coordinate grids and axes, workspace boundaries, trajectories, positions, physical limits, and future collision bounds. Robot-family composers produce these primitives before Desktop adapters translate them into WPF or Helix geometry. The Cartesian/XY schematic viewers and every mechanical-showcase motion-axis layer use this path first; remaining family-specific schematic overlays will migrate incrementally. Rendered meshes and overlay controls remain consumers, never sources, of robot or simulation state.
